        ...
        },
        {
          ...
          sessions: {
            WEEKEND_HEAVY: [
              ...
              { exercise: "PULL UP — BAND SUPPORT",         weight: "BW",   sets: "3", reps: "5" },
              { exercise: "CABLE BAR PULLS",                weight: "45",   sets: "3", reps: "12", note: "Cables broke" },
      ...
      weeks: [
        {
          ...
          sessions: {
            ...
            WEEKDAY_MEDIUM: [
              ...
              { exercise: "SINGLE ARM BICEP STRAP CURLS SS", weight: "7.5",  sets: "3", reps: "8" },
        ...
        },
        {
          ...
          sessions: {
            ...
            WEEKDAY_MEDIUM: [
              ...
              { exercise: "Preacher curls SS",               weight: "15",   sets: "3", reps: "10" },
              { exercise: "Tricep Long bar pushdowns SS",    weight: "25",   sets: "3", reps: "10" },
              { exercise: "SINGLE ARM BICEP STRAP CURLS SS", weight: "7.5",  sets: "3", reps: "10" },
...
function load() {
  try { const s = localStorage.getItem(STORAGE_KEY); return s ? JSON.parse(s) : INITIAL_DATA; } catch { return INITIAL_DATA; }
}
function persist(d) {
...
// ─── INLINE EDITABLE ──────────────────────────────────────────────────────────
function Editable({ value, onSave, style, placeholder = "…", multiline = false }) {
  ...
  const commit = () => { setEditing(false); if (val.trim() !== value) onSave(val.trim() || value); };

  const inputStyle = {
    background: "#1c1c1c", border: "1px solid #3B82F6", borderRadius: 4,
    color: "#f0f0f0", padding: "3px 8px", outline: "none",
  ...
  if (editing) {
    return multiline
      ? <textarea ref={ref} value={val} onChange={e => setVal(e.target.value)} onBlur={commit}
          ...
          style={{ ...inputStyle, resize: "none", minHeight: 32 }} rows={1} />
  ...
  return (
    <span onClick={() => setEditing(true)} title="Click to edit"
      style={{ cursor: "text", borderRadius: 3, padding: "2px 4px", display: "inline-block",
...
export default function GymDashboard() {
  ...
  const update = (newData, skipHistory = false) => {
    if (!skipHistory) setHistory(h => [...h.slice(-MAX_HISTORY), data]);
    setData(newData);
    persist(newData);
    showToast("✓ Saved");
  };
  ...
  const addBlock = () => {
    const d = clone();
    d.blocks.push({ block: `BLOCK ${d.blocks.length + 1}`, weeks: [] });
    update(d); setActiveBlockIdx(d.blocks.length - 1); setActiveWeek(0);
  };
  const deleteBlock = (bi) => {
    if (d.blocks.length === 1) return;
    const d = clone(); d.blocks.splice(bi, 1); update(d);
    setActiveBlockIdx(0); setActiveWeek(0);
  };
  ...
  const setSessionLabel = (bi, wi, sk, v) => {
    const d = clone();
    // rename key
    const week = d.blocks[bi].weeks[wi];
    const sessions = week.sessions;
    const dates = week.dates;
    const entries = Object.entries(sessions);
    const idx = entries.findIndex(([k]) => k === sk);
    const newKey = v.toUpperCase().replace(/\s+/g, "_");
    entries[idx] = [newKey, entries[idx][1]];
    week.sessions = Object.fromEntries(entries);
    week.dates[newKey] = dates[sk];
    delete week.dates[sk];
    update(d);
  };
  const addWeek = (bi) => {
    const d = clone();
    const weeks = d.blocks[bi].weeks;
    const template = weeks.length ? JSON.parse(JSON.stringify(weeks[weeks.length - 1])) : {
      label: "Week 1",
      dates: { WEEKEND_HEAVY: "DD/MM/YYYY", WEEKDAY_MEDIUM: "DD/MM/YYYY", WEEKDAY_HEAVY: "DD/MM/YYYY" },
      sessions: { WEEKEND_HEAVY: [], WEEKDAY_MEDIUM: [], WEEKDAY_HEAVY: [] },
    };
  ...
  // ── Exercise edits ──
  const setExField = (bi, wi, sk, ei, field, v) => {
    const d = clone(); d.blocks[bi].weeks[wi].sessions[sk][ei][field] = v; update(d);
  };
  ...
  const deleteExercise = (bi, wi, sk, ei) => {
    const d = clone(); d.blocks[bi].weeks[wi].sessions[sk].splice(ei, 1); update(d);
  };
    ...
    <div style={{ minHeight: "100vh", background: "#080808", fontFamily: "'Inter','Helvetica Neue',Arial,sans-serif", color: "#e8e8e8" }}>
      ...
      {toast && (
        <div style={{
          ...
          background: "#111", border: `1px solid ${toast.color}`,
          color: toast.color, padding: "10px 20px", borderRadius: 8,
      ...
      <div style={{ borderBottom: "1px solid #1a1a1a", padding: "24px 32px 0", background: "#0a0a0a" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 14 }}>
            <Editable value={data.logName} onSave={setLogName}
              style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.02em", color: "#f0f0f0" }} />
            <span style={{ color: "#252525", fontSize: 18 }}>·</span>
            <Editable value={data.programmeName} onSave={setProgrammeName}
              style={{ fontSize: 14, letterSpacing: "0.1em", color: "#555", textTransform: "uppercase" }} />
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <span style={{ fontSize: 11, color: "#333", letterSpacing: "0.06em" }}>Click any text to edit</span>
            <button onClick={undo} disabled={!history.length}
              style={{
                padding: "8px 18px", background: history.length ? "#181818" : "#0f0f0f",
                color: history.length ? "#a78bfa" : "#2a2a2a",
                border: `1px solid ${history.length ? "#2e2e2e" : "#161616"}`,
                borderRadius: 6, cursor: history.length ? "pointer" : "default",
                fontSize: 13, fontWeight: 600, letterSpacing: "0.04em",
                transition: "all 0.15s",
              }}
              onMouseEnter={e => history.length && (e.currentTarget.style.background = "#1f1a2e")}
              onMouseLeave={e => e.currentTarget.style.background = history.length ? "#181818" : "#0f0f0f"}
            >↩ Undo</button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "flex-end", flexWrap: "wrap" }}>
          {blocks.map((b, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 0 }}>
              <button onClick={() => { setActiveBlockIdx(i); setActiveWeek(0); }}
                style={{
                  padding: "11px 22px", borderRadius: "7px 0 0 0",
                  background: activeBlockIdx === i ? "#e8e8e8" : "transparent",
                  color: activeBlockIdx === i ? "#080808" : "#444",
                  border: "none", cursor: "pointer",
                  fontSize: activeBlockIdx === i ? 14 : 13,
                  fontWeight: activeBlockIdx === i ? 700 : 400,
                  letterSpacing: "0.07em", textTransform: "uppercase",
                }}>
                {activeBlockIdx === i
                  ? <Editable value={b.block} onSave={v => setBlockName(i, v)}
                      style={{ fontSize: 14, fontWeight: 700, color: "#080808" }} />
                  : b.block}
              </button>
              {activeBlockIdx === i && blocks.length > 1 && (
                <button onClick={() => deleteBlock(i)}
                  style={{ padding: "11px 8px 11px 0", background: "#e8e8e8", border: "none", cursor: "pointer", color: "#888", fontSize: 13, borderRadius: "0 7px 0 0" }}
          ...
          <button onClick={addBlock}
            style={{ padding: "11px 16px", background: "transparent", color: "#2a2a2a", border: "none", cursor: "pointer", fontSize: 13, borderRadius: "6px 6px 0 0" }}>
            + Block
          </button>
        </div>
      </div>
          ...
          {weeks.map((week, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 0 }}>
              <button onClick={() => setActiveWeek(i)}
                style={{
                  padding: "9px 18px",
                  background: activeWeek === i ? "#1c1c1c" : "transparent",
                  color: activeWeek === i ? "#e8e8e8" : "#404040",
                  border: `1px solid ${activeWeek === i ? "#2e2e2e" : "#181818"}`,
                  borderRight: activeWeek === i ? "none" : `1px solid ${activeWeek === i ? "#2e2e2e" : "#181818"}`,
                  borderRadius: weeks.length > 1 && activeWeek === i ? "6px 0 0 6px" : "6px",
                  cursor: "pointer", fontSize: 13, letterSpacing: "0.06em",
                  fontWeight: activeWeek === i ? 600 : 400,
                }}>
                {activeWeek === i
                  ? <Editable value={week.label} onSave={v => setWeekLabel(activeBlockIdx, i, v)}
                      style={{ fontSize: 13, fontWeight: 600, color: "#e8e8e8" }} />
                  : week.label}
              </button>
              {activeWeek === i && weeks.length > 1 && (
                <button onClick={() => deleteWeek(activeBlockIdx, i)}
                  style={{ padding: "9px 10px", background: "#1c1c1c", border: "1px solid #2e2e2e", borderLeft: "none", borderRadius: "0 6px 6px 0", cursor: "pointer", color: "#333", fontSize: 14 }}
                  onMouseEnter={e => e.currentTarget.style.color = "#EF4444"}
                  onMouseLeave={e => e.currentTarget.style.color = "#333"}>×</button>
              )}
            </div>
          ))}
          <button onClick={() => addWeek(activeBlockIdx)}
            style={{ padding: "9px 16px", background: "transparent", color: "#2a2a2a", border: "1px dashed #1e1e1e", borderRadius: 6, cursor: "pointer", fontSize: 13 }}>
            + Week
          </button>
        </div>
            ...
            return (
              <div key={sessionKey} style={{ background: "#0e0e0e", border: "1px solid #1a1a1a", borderRadius: 12, overflow: "hidden" }}>
                ...
                <div style={{ padding: "16px 20px", borderBottom: `2px solid ${config.accent}`, background: "#0b0b0b" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: config.accent }}>
                        {config.label}
                      </span>
                    </div>
                    <Editable value={sessionDate} onSave={v => setDate(activeBlockIdx, activeWeek, sessionKey, v)}
                      style={{ fontSize: 13, color: "#484848", letterSpacing: "0.04em" }}
                ...
                <div style={{ padding: "4px 0" }}>
                  {exercises.map((ex, ei) => (
                    <div key={ei} style={{
                      display: "grid", gridTemplateColumns: "1fr 70px 95px 28px",
                      alignItems: "center", padding: "10px 20px",
                      borderBottom: ei < exercises.length - 1 ? "1px solid #121212" : "none", gap: 8,
                    }}>
                      <div>
                        <Editable value={ex.exercise} onSave={v => setExField(activeBlockIdx, activeWeek, sessionKey, ei, "exercise", v)}
                          style={{ fontSize: ex.isBackOff ? 12 : 14, color: ex.isBackOff ? "#383838" : "#d0d0d0", fontStyle: ex.isBackOff ? "italic" : "normal" }}
                          placeholder="Exercise name" />
                        {ex.note && <div style={{ fontSize: 10, color: "#F59E0B", letterSpacing: "0.08em", textTransform: "uppercase", marginTop: 3 }}>⚠ {ex.note}</div>}
                      </div>
                      <Editable value={String(ex.weight)} onSave={v => setExField(activeBlockIdx, activeWeek, sessionKey, ei, "weight", v)}
                        style={{ fontSize: 15, fontWeight: 700, color: ex.isBackOff ? "#2a2a2a" : "#f0f0f0", fontVariantNumeric: "tabular-nums" }}
                        placeholder="kg" />
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        <Editable value={String(ex.sets)} onSave={v => setExField(activeBlockIdx, activeWeek, sessionKey, ei, "sets", v)}
                          style={{ fontSize: 13, color: "#606060" }} placeholder="sets" />
                        <span style={{ fontSize: 11, color: "#252525" }}>×</span>
                        <Editable value={String(ex.reps)} onSave={v => setExField(activeBlockIdx, activeWeek, sessionKey, ei, "reps", v)}
                          style={{ fontSize: 13, color: "#606060" }} placeholder="reps" />
                      </div>
